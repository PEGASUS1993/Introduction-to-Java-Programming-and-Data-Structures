...
public class Exercise29_03Extra extends Application {
  private Button btShowMST = new Button("Show MST");
  private Button btShowAllShortestPath = new Button("Show All SP from Source");
  private Button btShowShortestPath = new Button("Show Shortest Path");
  private TextField tfSourceVertex = new TextField();
  private TextField tfSourceVertexForShortestPath = new TextField();
  private TextField tfEndingVertexForShortestPath = new TextField();
  ...
  public void start(Stage primaryStage) {   
    ...
    hBox1.getChildren().addAll(new Label("Starting vertex: "), tfSourceVertex, btShowAllShortestPath);
    tfSourceVertex.setPrefColumnCount(2);
    tfSourceVertex.setAlignment(Pos.BASELINE_RIGHT);
    hBox1.setStyle("-fx-border-color: green");
    
    HBox hBox2 = new HBox(5);
    hBox2.getChildren().addAll(new Label("Source vertex: "), tfSourceVertexForShortestPath, 
      new Label("Ending vertex: "), tfEndingVertexForShortestPath, btShowShortestPath);
    tfSourceVertexForShortestPath.setPrefColumnCount(2);
    tfEndingVertexForShortestPath.setPrefColumnCount(2);
    tfSourceVertexForShortestPath.setAlignment(Pos.BASELINE_RIGHT);
    tfEndingVertexForShortestPath.setAlignment(Pos.BASELINE_RIGHT);
    hBox2.setStyle("-fx-border-color: green");
    
    HBox hBox = new HBox(15);
    hBox.setAlignment(Pos.CENTER);
    hBox.getChildren().addAll(btShowMST, hBox1, hBox2);
    
    pane.setBottom(hBox);
    
    // Create a scene and place it in the stage
    Scene scene = new Scene(pane, 950, 350);
    primaryStage.setTitle("Exercise29_03"); // Set the stage title
    primaryStage.setScene(scene); // Place the scene in the stage
    primaryStage.show(); // Show the stage
    
    btShowMST.setOnAction(e -> view.displayMST());
    btShowAllShortestPath.setOnAction(e -> view.displayAllShortestPath());
    btShowShortestPath.setOnAction(e -> view.displayShortestPath(
        Integer.parseInt(tfSourceVertexForShortestPath.getText()), 
        Integer.parseInt(tfEndingVertexForShortestPath.getText())));
  }
  ...
  class View extends Pane {
    ...
    final String[] instructions = { "INSTRUCTIONS", "Add a vertex:", "Left Click Mouse", "Remove a vertex:",
        "Right Click Mouse", "Move a Vertex:", "Move Mouse + Ctrl Button", "Connect:", 
        "Click a vertex, move mouse,\nand click another vertex" };
    ...
    void drawEdges() {
      ...
      for (int i = 0; i < edges.size(); i++) {
        ...
        getChildren().add(new Text((edges.get(i).u.getX() + edges.get(i).v.getX()) / 2, 
            (edges.get(i).u.getY() + edges.get(i).v.getY()) / 2, "" + Math.round(edges.get(i).u.getDistance(edges.get(i).v))));     
      }
    }
    ...
    public WeightedGraph<Vertex> getGraph() {     
      ...
      for (Edge e : this.edges) {
        ...
        edges.add(new WeightedEdge(x, y, e.u.getDistance(e.v)));
        edges.add(new WeightedEdge(y, x, e.u.getDistance(e.v)));
      }
      
      // Create a graph
      WeightedGraph<Vertex> graph = new WeightedGraph<>(list, edges);
      return graph;
    }
    
    public void displayMST() {     
      WeightedGraph<Vertex>.SearchTree tree = getGraph().getMinimumSpanningTree();

      repaint();
      for (int i = 0; i < tree.getNumberOfVerticesFound(); i++) {
        int y = tree.getParent(i);
        if (y != -1) {
          drawArrowLine(list.get(y).getX(), list.get(y).getY(), list.get(i).getX(), list.get(i).getY());
        }
      }
    }
    
    public void displayShortestPath(int startingV, int endingV) {     
      WeightedGraph<Vertex>.SearchTree tree = getGraph().getShortestPath(startingV);
    ...
    public void displayAllShortestPath() {     
      WeightedGraph<Vertex>.SearchTree tree = 
        getGraph().getShortestPath(Integer.parseInt(tfSourceVertex.getText()));

      repaint();
      for (int i = 0; i < tree.getNumberOfVerticesFound(); i++) {
        int y = tree.getParent(i);
        if (y != -1) {
          drawArrowLine(list.get(y).getX(), list.get(y).getY(), 
