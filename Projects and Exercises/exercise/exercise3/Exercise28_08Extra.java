...
public class Exercise28_08Extra extends Application {
  private Button btTestBipartite = new Button("Is the graph bipartite? ");
  ...
  public void start(Stage primaryStage) {   
    ...
    hBox.getChildren().addAll(btTestBipartite, text);
    
    pane.setBottom(hBox);
    
    // Create a scene and place it in the stage
    Scene scene = new Scene(pane, 950, 350);
    primaryStage.setTitle("Exercise28_08"); // Set the stage title
    primaryStage.setScene(scene); // Place the scene in the stage
    primaryStage.show(); // Display the stage
    
    btTestBipartite.setOnAction(e -> view.isBipartite());
  }
  ...
  class View extends Pane {
    ...
    public UnweightedGraphTestBipartite<Vertex> getGraph() {     
      ...
      // Create a graph
      UnweightedGraphTestBipartite<Vertex> graph = new UnweightedGraphTestBipartite<>(list, edges);
      return graph;
    }
    
    public void isBipartite() {     
      if (getGraph().isBipartite()) {
        text.setText("Yes");
      }
      else {
        text.setText("No");
