...
public class Exercise29_13 extends Application {
  ...
  public void start(Stage primaryStage) {
    HBox hBox = new HBox();
    hBox.getChildren().addAll(new Label("Starting City:"),
      tfStartCity, new Label("Ending City:"), tfEndCity, btSP);
    hBox.setAlignment(Pos.CENTER);
    
    Group n = new Group(view);
    n.setScaleX(1.5);
    n.setScaleY(1.5);
    ...
    btSP.setOnAction( e -> {
      String name1 = tfStartCity.getText();
      int index1 = graph1.getIndex(new City(name1, 0, 0));
      if (index1 < 0) {
        lblStatus.setText(name1 + " is not in the map");
        return;
      }

      String name2 = tfEndCity.getText();
      int index2 = graph1.getIndex(new City(name2, 0, 0));
      if (index2 < 0) 
        lblStatus.setText(name2 + " is not in the map");
      else {
