...
public class Exercise22_23 extends Application {
  ...
  private int[] queens = {0, -1, -1, -1, -1, -1, -1, -1}; 
  ...
  public void start(Stage primaryStage) {
    ...
    for (int i = 0; i < list.size(); i++) {
      hBox.getChildren().add(getChessBoardWithLabel("Solution " + (i + 1), 
        list.get(i)));
    }
    
    // Create a scene and place it in the stage
    Scene scene = new Scene(new ScrollPane(hBox), 55 * SIZE, 55 * SIZE);
    primaryStage.setTitle("Exercise22_23"); // Set the stage title
  ...
  private boolean search() {
    ...
    while (k > -1) {
      ...
      } else { // k is actually 8
        // Print the current solution
        System.out.print("Solution " + ++count + ": ");
        printASolution(queens);

        list.add(queens.clone());
        k--;
      }
    }
  ...
  public void printASolution(int[] queens) {
    for (int column : queens) {
  ...
  private BorderPane getChessBoardWithLabel(String label, int[] queens) { 
    BorderPane pane = new BorderPane();
    pane.setCenter(getChessBoard(queens));
    Label lblCount = new Label(label);
    pane.setTop(lblCount);
    BorderPane.setAlignment(lblCount, Pos.CENTER);
    return pane;
  }

  private GridPane getChessBoard(int[] queens) {
    // Display chess board
