...
public class Exercise28_25 {
  ...
  public static class UnweightedGraph<V> implements Graph<V> {
    ...
    public boolean remove(V v) {
      ...
      if (index > 0) {
        // Remove every edge that is incident to index 
        ...
        // Remove the vertex and re-organize index
    ...
    private void removeEdge(int i, int index) {
      List<Edge> list = neighbors.get(i);
      for (int k = 0; k < list.size(); k++) {
        if (list.get(k).v == index) {
          list.remove(k);
          break;
        }
      }
