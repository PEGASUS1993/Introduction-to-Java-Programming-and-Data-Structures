...
public class Exercise28_09 {
  public static void main(String[] args) {
    ...
    UnweightedGraphGetBipartiteSets<String> graph = 
      new UnweightedGraphGetBipartiteSets<>(vertices, edges);
      
    System.out.println("The bipartite sets are " + graph.getBipartite());
  }
}

class UnweightedGraphGetBipartiteSets<V> extends UnweightedGraph<V> {
  /** Construct an empty graph */
  public UnweightedGraphGetBipartiteSets() {
  }
    
  /** Construct a graph from vertices and edges stored in arrays */
  public UnweightedGraphGetBipartiteSets(V[] vertices, int[][] edges) {
    super(vertices, edges);
  }

  /** Construct a graph from vertices and edges stored in List */
  public UnweightedGraphGetBipartiteSets(List<V> vertices, List<Edge> edges) {
    super(vertices, edges);
  }

  /** Construct a graph for integer vertices 0, 1, 2 and edge list */
  public UnweightedGraphGetBipartiteSets(List<Edge> edges, int numberOfVertices) {
    super(edges, numberOfVertices);
  }
  
  /** Construct a graph from integer vertices 0, 1, and edge array */
  public UnweightedGraphGetBipartiteSets(int[][] edges, int numberOfVertices) {
    super(edges, numberOfVertices);
  }
  ...
  */
  public List<List<Integer>> getBipartite() {
    ...
    List<List<Integer>> bipartites = new ArrayList<>();
    bipartites.add(new ArrayList<Integer>());
    bipartites.add(new ArrayList<Integer>());
    
    for (int i = 0; i < vertices.size(); i++) {
      if (depth[i] % 2 == 0) {
        bipartites.get(0).add(i);
      }
      else {
        bipartites.get(1).add(i);
      }
    }
