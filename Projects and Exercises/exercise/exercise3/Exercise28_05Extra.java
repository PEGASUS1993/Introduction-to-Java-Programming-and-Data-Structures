...
public class Exercise28_05Extra extends Application {
  private Button btDisplayCycle = new Button("Find a Cycle");
  ...
  public void start(Stage primaryStage) {   
    ...
    hBox.getChildren().addAll(btDisplayCycle, text);
    
    pane.setBottom(hBox);
    
    // Create a scene and place it in the stage
    Scene scene = new Scene(pane, 950, 350);
    primaryStage.setTitle("Exercise28_05"); // Set the stage title
    primaryStage.setScene(scene); // Place the scene in the stage
    primaryStage.show(); // Display the stage
    
    btDisplayCycle.setOnAction(e -> view.displayCycle());
  }
  ...
  class View extends Pane {
    ...
    public UnweightedGraphFindCycle<Vertex> getGraph() {     
      ...
      // Create a graph
      UnweightedGraphFindCycle<Vertex> graph = new UnweightedGraphFindCycle<>(list, edges);
      return graph;
    }
    
    public void displayCycle() {     
      List<Integer> path = getGraph().getACycle();
      
      if (path == null) {
        text.setText("No cylces");
        return;
      }
      repaint();
      ...
      for (int i = 0; i < path.size() - 1; i++) {
        int v1 = path.get(i);
        int v2 = path.get(i + 1);
        drawArrowLine(list.get(v1).getX(), list.get(v1).getY(), list.get(v2).getX(), list.get(v2).getY());
      }
      
      int v1 = path.get(0);
      int v2 = path.get(path.size() - 1);
      drawArrowLine(list.get(v2).getX(), list.get(v2).getY(), list.get(v1).getX(), list.get(v1).getY());
    }
